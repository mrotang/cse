(function(opts_) {
    ...
    var J = new k(l, "https://www.google.com/cse/static/style/look/%{versionDir}%{versionSlash}%{theme}.css"),
        ...
        B = new k(l, "https://www.google.com/cse/static/element/%{versionDir}%{versionSlash}cse_element__%{lang}.js"),
        L = new k(l, "/");
    ...
    window.__gcse.scb = function() {
        ...
        google.search.cse.element.init(opts_) && ("explicit" !== a.parsetags ? "complete" === document.readyState || "interactive" === document.readyState ? (google.search.cse.element.go(), null == b || b()) : google.setOnLoadCallback(function() {
    ...
    function O(a) {
        ...
        b.type = "text/css";
        a: {
            if (a instanceof p) b.href = r(a).toString();
            else {
                if (-1 === I.indexOf("stylesheet")) throw Error('TrustedResourceUrl href attribute required with rel="stylesheet"');
                if (a instanceof E) a = a instanceof E && a.constructor === E ? a.g : "type_error:SafeUrl";
                else {
                    c: if (H) {
                        try {
                            var c = new URL(a)
    ...
    S.src = r(P);
    var T, U, V, W = null == (V = (U = (S.ownerDocument && S.ownerDocument.defaultView || window).document).querySelector) ? void 0 : V.call(U, "script[nonce]");
    (T = W ? W.nonce || W.getAttribute("nonce") || "" : "") && S.setAttribute("nonce", T);
    ...
    if (M()) {
        ...
        var X, Y = "v" + (opts_.uiOptions.cssThemeVersion || 4);
        X = u(J, {
            versionDir: Y,
            versionSlash: Y ? L : "",
    ...
    // Custom script to add recommendations based on top search keywords
    function addSearchRecommendations() {
        var topSearchKeywords = opts_.uiOptions.topSearchKeywords || [];
        if (topSearchKeywords.length > 0) {
            var recommendationContainer = document.createElement('div');
            recommendationContainer.style.marginTop = '20px';
            recommendationContainer.style.padding = '10px';
            recommendationContainer.style.border = '1px solid #ccc';
            recommendationContainer.style.backgroundColor = '#f9f9f9';

            var title = document.createElement('h4');
            title.innerText = 'Top Searches:';
            recommendationContainer.appendChild(title);

            var list = document.createElement('ul');
            topSearchKeywords.forEach(function(keyword) {
    ...
    window.__gcse && window.__gcse.ct ? addSearchRecommendations() : window.__gcse = {
        ct: Date.now(),
        scb: addSearchRecommendations
    };

})({
    "cx": "28bf9b5e36cf02e60",
    "language": "id",
    "theme": "V2_DEFAULT",
    "uiOptions": {
        "resultsUrl": "",
        ...
        "webSearchExtendedRestricts": {
            ...
            "filter": 0,
            "sort": "",
            "as_oq": "kredit hp,cicilan,murah",
            "as_sitesearch": ""
        },
        "webSearchQueryAddition": "kredit hp,cicilan,murah",
